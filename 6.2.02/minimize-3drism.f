...
c
      call lbfgs3d (nvar,xx,minimum,grdmin,minimiz13d,optsave,gpu)
c
...
c
      if (analytic) then
         call gradient3d (minimum,e3d,derivs,1,gpu)
      else
         minimum = energy ()
...
c
      if (analytic) then
         call gradient3d (e,e3d,derivs,1,gpu)
      else
         e = energy ()
         call numgrad (energy,derivs,eps)
      end if
