...
c
c     get desired interval for solvation calculation
c
      if (.not. use_bounds) then
         num = -1
         call nextarg (string,exist)
         if (exist)  read (string,*,err=1190,end=1190)  num
 1190            continue
         if (num .le. 0) then
            write (iout,1200)
 1200       format (/,' Enter the Desired Interval for 3D-RISM',
     &           ' time [1] :  ',$)
            read (input,1210)  num
 1210       format (i10)
         end if
...
c
c     get desired GPU device
c
      if (.not. use_bounds) then
         gpu = -2
         call nextarg (string,exist)
         if (exist)  read (string,*,err=2190,end=2190)  gpu
 2190            continue
         if (gpu .lt. -1) then
            write (iout,2200)
 2200       format (/,' Enter the number of GPU',
     &           ' device [-1] :  ',$)
            read (input,2210)  gpu
 2210       format (i10)
         end if
...
c
      do istep = 1, nstep
         if (integrate .eq. 'VERLET') then
            ...
            call sdstep3d (istep,dt,num,gpu)
         else if (integrate .eq. 'BUSSI') then
            call bussi (istep,dt)
         else if (integrate .eq. 'NOSE-HOOVER') then
            call nose (istep,dt)
            ...
            call beeman3d (istep,dt,num,gpu)
         end if
      end do
c
